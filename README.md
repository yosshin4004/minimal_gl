...
PC Intro とは、Windows PC 上で動作する数キロバイト程度の小さな実行ファイルで、音と映像を生成し、その表現内容で競うメガデモ（デモシーン）のカテゴリです。  
...
- shader_minifier.exe と crinkler.exe  
	https://github.com/laurentlb/Shader_Minifier/releases  (ver1.6.0 で動作確認済み)  
	http://www.crinkler.net/  (ver3.0 で動作確認済み)  
...
1. シェーダファイルを用意する  
	examples/ 以下の適当な *.gfx.glsl *.snd.glsl ファイルをテンプレートとして利用してください。
...
4. シェーダのエディット  
	MinimalGL にはシェーダのエディタ機能はありません。
	シェーダのエディットはお使いのテキストエディタ等でシェーダファイルを直接書き換えることで行います。  
...
6. ファイルサイズ削減（minify）
	...
	描画設定（[Setup]→[Render Settings]）から不要な機能をオフにすることでもファイルサイズを削減できます。
...
- ユーザーテクスチャ  
	...
	テクスチャは最大 4 つまで登録可能です。なおこの機能はエクスポートされた実行ファイル上では利用できません。

- パラメータバインド  
	uniform パラメータを ImGui のウィジェットまたは MIDI コントローラにバインドし、リアルタイムに入力を反映しながら結果を確認できます。
...
- twigl と動作が異なる  
	...
	- uniform float s を利用している  
		...
		MinimalGL の twigl 互換サンプルでは（処理負荷上の問題から）平均を取っていません。
		そのため uniform float s に依存したエフェクトの動作は互換になりません。

	- 未定義の動作が含まれる or 未初期化変数が存在する  
		環境によって異なる結果が生成される場合があります。
